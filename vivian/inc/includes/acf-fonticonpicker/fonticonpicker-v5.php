...
class acf_field_fonticonpicker extends acf_field {
	...
	function __construct() {
		...
		$this->label = esc_html__('Icon Picker', 'vivian');
		$this->category = esc_html__("jQuery", 'vivian');
		$this->defaults = array(
			'allow_null' 	=>	0
		);
		$this->l10n = array();
	...
	function render_field( $field ) {
		...
		if ( !isset( $this->json_content['icons'] ) ){
			esc_html_e('No icons found', 'vivian');
			return;
		}

		// icons SELECT input
		echo '<select name="'. $field['name'] .'" id="'. $field['name'] .'" class="acf-iconpicker">';
		echo '<option value="">'. esc_html__('None', 'vivian').'</option>';
