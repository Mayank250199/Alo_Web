<?php
if( ! defined('Vivian_WIDGET_FIELDS_DIR') ) { define('Vivian_WIDGET_FIELDS_DIR', get_template_directory_uri() . '/inc/includes/subsolar-widget-fields'); }
/**
*  Subsolar Designs helper functions for creating native widget fields
*/

class Vivian_Widget_Fields {
	...
	static function admin_enqueue_scripts($hook){
		...
		wp_enqueue_script(
			'widget-selectize-js',
			Vivian_WIDGET_FIELDS_DIR . '/assets/js/selectize.min.js',
			array( 'jquery' ),
			'1.0',
			true
			);
		...
		wp_enqueue_script(
			'ssdwf-js',
			Vivian_WIDGET_FIELDS_DIR . '/assets/js/scripts.js',
			array( 'jquery' ),
			'1.0',
			true
			);
		...
		wp_enqueue_style(
			...
			Vivian_WIDGET_FIELDS_DIR . '/assets/css/selectize.custom.css',
			array(),
			'1.0'
			);
	}
	...
	static function field($args = array()){
		...
		switch ($args['type']) {
			...
			?>	
				<label for="<?php echo esc_attr($args['name']); ?>"><?php echo esc_attr($args['label']); ?></label>
				<select id="<?php echo esc_attr($args['id']); ?>" class="ssdwf-multi-select" name="<?php echo esc_attr($args['name']); ?>" data-name="<?php echo esc_attr($args['name']); ?>">
					<option value=""><?php esc_html_e('Select...', 'vivian') ?></option>
...
add_action( 'widgets_init', array( 'Vivian_Widget_Fields', 'init' ) );